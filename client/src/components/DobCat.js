...
function DobCat() {
	const [category, setCategories] = useState([]) //массив айтем который мы вводим - items
	...
	return (
		<>
			...
			></Table>
			<Modal
				title={categoryRecord.ID_Category ? 'Изменение сущности с ID_Category=' + categoryRecord.ID_Category : 'Добавление новой сущности'}
				open={modalVisible}
				okText='Сохранить'
				cancelText='Отмена'
				onCancel={() => close()}
				centered
				footer={[
					<Button type='primary' onClick={() => saveTort()} disabled={!categoryRecord.ID_Category && !categoryRecord.Name}>
						Сохранить
					</Button>,
			...
			>
				<Form labelAlign='left' labelCol={{ span: 4 }} wrapperCol={{ span: 18 }}>
					<Form.Item label='ID_Category'>
						<Input
							value={categoryRecord.ID_Category}
						/>
					</Form.Item>
